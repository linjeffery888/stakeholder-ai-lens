...
class LLM:
    ...
    def best_match(self, new_desc: str, candidates: list) -> dict:
        ...
        system = (
            ...
            "null.\n"
            "MATCH requires BOTH: (1) the same mechanism/action AND (2) the same "
            "OBJECT it acts on. The same mechanism on a DIFFERENT object is NOT a "
            "match. For example, reconciling financial accounts, reconciling "
            "inventory counts, reconciling identity labels, and matching purchase "
            "orders are all 'reconciliation' but act on DIFFERENT objects, so they "
            "are SEPARATE use cases and must NOT be merged. Likewise maintaining "
            "master data is NOT account reconciliation.\n"
            "Crossing departments is fine ONLY if the object is truly the same "
            "(e.g. 'assemble a status picture from multiple systems' is one use "
            "case across functions). But do NOT pool different objects under one "
            "abstract mechanism.\n"
            ...
            "confidence is your certainty they are the same problem AND object.\n"
